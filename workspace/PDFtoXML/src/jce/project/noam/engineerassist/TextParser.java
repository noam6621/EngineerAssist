...
public class TextParser {
	...
	private boolean wasDescriptionFound;
...
//				parseDesignMetric();
	...
	/**
	 * Parse the global variable text and write the different fields to file.
	 * 
	 * @throws IOException If there is an error writing to file.
	 */
	...
	private boolean updateGlobalSection() {
		...
		if (currentLine.trim().startsWith(Constants.GLOBAL_VARIABLE_TYPE) || currentLine.trim().startsWith(Constants.GLOBAL_VARIABLE_TYPE2)) {
			...
			default: {
				if (!wasDescriptionFound & Character.isUpperCase(currentLine.charAt(0))) {
					currentSection = Constants.GLOBAL_VARIABLE_DESCRIPTION_SECTION;
					wasDescriptionFound = true;
					wasUpdated = true;
				}
				else {
					wasUpdated = false;
